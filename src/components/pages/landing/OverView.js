                ...
                if (data) {
                    ...
                    Web3.eth.getBlockNumber(function (error, result) {
                        if (result) {
                            ...
                            let count = result - 500
                            // let latestBlock = result - count
                            // console.log("result", count, result);

                            for (let i = count; i <= result; i++) {
                                // console.log("check", i);
                                Web3.eth.getBlock(i)
                                    .then((result) => {
                                        if (result) {
                                            if (result.transactions) {
                                                ...
                                                if (result.transactions.length >= 1) {
                                                    for (let i = 0; i <= result.transactions.length; i++) {
                                                        if(result.transactions[i] !== undefined){
                                                            // console.log(`result.transactions[${i}]`, result.transactions[i]);
                                                            Web3.eth.getTransaction(result.transactions[i].toString()).then(
                                                                (e) => {
                                                                    if (e) {
