...
var VERIFY_CONTRACT = process.env.REACT_APP_VERIFY_CONTRACT;
...
function VerifyPublish() {
  ...
  const submitHander = async () => {
    ...
    if (contractName === "" || contractName === undefined) {
      return $("#contractNameErr").show()
    }
    if (code === '') {
      return $("#souceErr").show()
    }

    if (optimization === "") {
      return $("#optimization").show()
    }


    var formdata = new FormData();

    formdata.append("sourcescode", code)
    formdata.append("txn_address", _contractAddress)
    formdata.append("solidity_contract_name", contractName)
    formdata.append("solidity_version", compilerVersion)
    formdata.append("optimizer", optimization)

    setLoading(true)
    const { data } = await axios.post(VERIFY_CONTRACT, formdata)
    console.log("data", data)
    if (data.statusMsg === "verify") {
      setLoading(false)
      toast.success('Contract verify', {
        position: "top-center",
        autoClose: 5000,
        theme: "colored",
            ...
            <Col lg={12}>
              <div className='verify_contract_form'>
                ...
                <Row className="mb-3 mt-3">
                  ...
                  <Form.Group as={Col} controlId="formGridZip">
                    ...
                    <Form.Select aria-label="Optimization" onChange={(e) => setOptimization(e.target.value)}>
                      <option>select optimization</option>
                      <option value="1">Yes</option>
                      <option value="0">No</option>
                    </Form.Select>
                    <p id="optimization" className='error_msg'>*Please select optimization</p>
                  </Form.Group>
                </Row>
                ...
                <div className='verify_public_btton'>
                  <Button className='primary_btn' onClick={submitHander}> {loading ? <>loading...</> : <>Verify and Public</>}</Button>
