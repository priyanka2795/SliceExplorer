...
export default function BlockTransaction() {
    ...
    var from = location.pathname.split("/block/txs/")[1];
                ...
                if (getTransactions.transactions.length > 0) {
                    ...
                    for (let i = 0; i < getTransactions.transactions.length; i++) {
                        Web3.eth.getTransaction(getTransactions.transactions[i]).then((contractAdd) => {
                            ...
                            if (contractAdd.blockNumber === Number(from)) {
                                setLoad(false)
                            ...
                            if (contractAdd.to === null) {
                                ...
                                getContractAddressForEmptyTo(contractAdd.hash)
                                Web3.eth.getBlock(contractAdd.blockNumber).then((timeStampPara) => {
                                    // console.log("Contract Add", timeStampPara);
    ...
    function ContractIndexValue1(ContractAddressString) {
        // setIndexValue(a => [ContractAddressString, ...a])
        IndexValue1.push(ContractAddressString)
        // console.log("IndexValue", IndexValue.length);
    ...
    return (
        <>
            <section className='block_transaction'>
                <div className='container'>
                    <div className='row'>
                        <div className='col-lg-12'>
                            <div className='head_section'>
                                <h4>Transactions</h4>
                                <p>For Block <Link to={`/block_details/block/${from}`}>{from}</Link></p>
                            </div>
                        </div>
                    </div>
                    <div className='row'>
                        <div className='col-lg-12'>
                            <div>
                                <p>A total of {currentItemsCollections.length} transactions found</p>
                            </div>
                            {
                                load ?
                                        ...
                                        <section className='addresstransaction_table_section'>
                                            ...
                                            <Table responsive hover>
                                                ...
                                                <tbody>
                                                    ...
                                                    :
                                                    e.from === e.to ?
                                                        <div className='self_icon'>SELF</div>
                                                        :
                                                        <div className='out_icon'>OUT</div>
                                                            ...
                                                            <td>
...

                                                                {
                                                                    (e.to === null)
                                                                        ?
                                                                        <a href={`/address/${ContractAddNull[ContractIndexValue1(e.hash) - 1]}`} className='ContractAddress_toNull'>
